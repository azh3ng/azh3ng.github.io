...
1. 类中所有构造方法上都没有 `@Autowired` 注解
    ...
    2. 只有一个有参的构造方法，选用此构造方法，通过 [依赖注入#依赖查找](https://azh3ng.com/2022/01/14/Spring-Dependency-Injection.html#%E4%BE%9D%E8%B5%96%E6%9F%A5%E6%89%BE) 找到对应的入参 Bean
...
2. 类中存在 `@Autowired` 注解的构造方法（`@Autowired` 的 `required` 属性默认为 true）
    ...
    3. 所有被  `@Autowired` 标注的构造方法的 `required` 属性都为 false，继续判断
        ...
        2. 当调用 [getBean()](https://azh3ng.com/2022/01/10/Spring-BeanFactory-getBean.html) 时，有传入构造参数，或者在 BeanDefinition 中有设置构造参数，则确定构造参数个数，剔除所有参数个数小于此数量的构造方法
