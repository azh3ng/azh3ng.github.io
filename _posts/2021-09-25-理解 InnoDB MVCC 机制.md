...
以此类推，从上表可以看出，不同事务对一行记录的修改，通过 Undo log 的连接，使其近似于一条链表，链表的头就是最新数据，下一个节点就是上一个版本的旧数据，链表的尾部就是数据的最早的旧数据。
...
此时`事务Q`的 Read View 如下（与[场景一：Read Commited](#场景一：Read Commited)相同）：
...
11. 根据数据的`DB_ROLL_PTR`找到上个版本的数据：
