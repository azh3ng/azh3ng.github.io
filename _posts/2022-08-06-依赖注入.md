...
2. 遍历当前类的所有的**属性**字段 Field
  ...
  2. 如果字段是 static 的，则跳过，不进行注入，[[Spring对static字段不支持自动注入的原因|原因]]
...
3. 遍历当前类的所有**方法** `Method`
  ...
  3. 如果方法是 static 的，则跳过，不进行注入，[[Spring对static字段不支持自动注入的原因|原因]]
...
2. 遍历当前类的所有的**属性**字段 Field
    ...
    2. 判断是否为 `static` 字段，如果字段是 `static` 的，抛出异常（[[Spring对static字段不支持自动注入的原因|原因]]）
...
3. 遍历当前类的所有的**方法** `method`
    ...
    2. 判断是否为 `static` 方法，如果字段是 `static` 的，抛出异常（[[Spring对static字段不支持自动注入的原因|原因]]）
    3. 判断方法参数有且仅有一个，否则报错
    4. [[#CommonAnnotationBeanPostProcessor ResourceElement 构造方法|构造]] `CommonAnnotationBeanPostProcessor.ResourceElement`，并将其添加到 `currElements` 集合中
...
无论字段注入还是 Set 方法注入，都会调用 `InjectionMetadata.inject()` -> `InjectedElement.inject()` -> `ResourceElement.getResourceToInject()` 方法寻找 bean 用来注入
 ...
 2. 否则调用 `resolveDependency()` 进行[[#依赖查找]]
...
4. 判断 Bean 是否可以被用来注入
   ...
   2. [[#依赖注入中泛型注入的实现|泛型注入]] ：判断当前 `requiredType` 是不是泛型，如果是泛型，则把容器中所有的 beanName 找出来的，并获取到泛型的真正类型，然后进行匹配，如果当前 beanName 和当前泛型对应的真实类型匹配，那么则继续判断
   3. [[#Qualifier判断]] ：如果当前 DependencyDescriptor 上存在 `@Qualifier` 注解，那么则要判断当前 beanName 上是否定义了 `@Qualifier`，并且是否和当前 DependencyDescriptor 上的 `@Qualifier` 的值相等，相等则匹配
